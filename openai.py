...
def chat():
    ...
    if is_relevant_question(user_input):
        try:
            response_json = get_openai_response(user_input)

            if "error" in response_json:
                return jsonify({"reply": response_json["error"], "language": language, "show_icons": False}), 500

            bot_response = response_json.get("choices", [{}])[0].get("text", "")
...
def continue_response():
    ...
    try:
        response_json = get_openai_response(continue_prompt)

        if "error" in response_json:
            return jsonify({"reply": response_json["error"], "show_icons": False}), 500

        continuation = response_json.get("choices", [{}])[0].get("text", "")
