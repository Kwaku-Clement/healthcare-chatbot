...
def get_gemini_response(user_prompt):
    try:
        ...
        system_prompt = (
            ...
            "Respond in JSON format with a 'response' key containing your answer."
        )

        full_prompt = f"{system_prompt}\n\nUser: {user_prompt}\n\nResponse:"

        logging.info(f"Sending request to Gemini API with prompt: {full_prompt[:100]}...")

        response = model.generate_content(full_prompt)

        logging.info(f"Received response from Gemini API: {response.text[:100]}...")
        ...
        return {"error": f"Failed to get a response from the Gemini API. {str(e)}"}
...
def chat():
    try:
        ...
        app.logger.info(f"Received message: {user_input}, Language: {language}")

        if language == 'ak':
            translator = GoogleTranslator(source='ak', target='en')
            user_input = translator.translate(user_input)
            app.logger.info(f"Translated input: {user_input}")

        if is_relevant_question(user_input):
            response = get_gemini_response(user_input)
            app.logger.info(f"Gemini response: {response}")

            if "error" in response:
                return jsonify({"reply": response["error"], "language": language, "show_icons": False}), 500

            bot_response = response.get("response", "")
            formatted_response = format_response(bot_response)
            app.logger.info(f"Formatted response: {formatted_response}")

            if language == 'ak':
                translator = GoogleTranslator(source='en', target='ak')
                formatted_response = translator.translate(formatted_response)
                app.logger.info(f"Translated response: {formatted_response}")
            ...
            return jsonify({
                "error": "The question is not related to pregnancy or health problems. Please ask a relevant question.",
                "language": language,
                "show_icons": False
            }), 400
        ...
        return jsonify({
            "error": "It appears that an unexpected error has occurred. We apologize for the inconvenience. Please try again later of refresh the page.",
            "language": language,
            "show_icons": False
        }), 500
...
def regenerate():
    ...
    if is_relevant_question(user_input):
        try:
            response_json = get_gemini_response(user_input)
...
def continue_response():
    ...
    try:
        response_json = get_gemini_response(continue_prompt)

        if "error" in response_json:
            return jsonify({"reply": response_json["error"], "show_icons": False}), 500

        continuation = response_json.get("response", "")
